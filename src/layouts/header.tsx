...
const Header = () => {
  ...
  return (
    <div className={`fixed top-0 z-50 w-full bg-white left-0`}>
      <Container className="px-32 desktop:px-12 tablet:px-8 mobile:px-6 final:px-4">
        <div className="flex items-center justify-between py-2" id="product-banner">
          ...
          <div className="w-[40%] flex items-center justify-between mac:w-[60%] tablet:hidden">
            {MENU_ITEMS.map((item) => {
              if (item.type === "text") {
                return (
                  <Link href={item.href} key={`menu-${item.id}`}>
                    <p className="text-[#BEBEBE] hover:text-main-900 transition duration-300 text-[25px] font-500 cursor-pointer laptop:text-[15px]">
                      {item.label}
                    </p>
                  </Link>
                );
                ...
                if (item.type === "text") {
                  return (
                    <Link href={item.href} key={`menu-${item.id}`} onClick={() => setSidebar(false)}>
                      <p className="text-[#BEBEBE] hover:text-main-900  transition duration-300 text-[20px] font-500 cursor-pointer">
                        {item.label}
                      </p>
                    </Link>
                  );
